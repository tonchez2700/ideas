...
export const CustomAppoint = ({ data }: any) => {

    const colorsState = () => {

        let color
        switch (true) {
            case data.appointments.appointments_percentage <= 20:
                color = '#FF3729'
                break;
            case data.appointments.appointments_percentage <= 40:
                color = '#DAF7A6'
                break;
            case data.appointments.appointments_percentage <= 60:
                color = '#FF7F27'
                break;
            case data.appointments.appointments_percentage <= 80:
                color = '#F3DC06'
                break;
            case data.appointments.appointments_percentage == 100:
                color = 'green'
                break;
        }
        return color
            ...
            <AccordionItem
                    ...
                    <View style={{ flexDirection: 'row', height: '100%' }}>
                        <View style={{ alignItems: 'center', flexDirection: 'row', height: '100%' }}>
                            ...
                            <View style={{ paddingVertical: 10 }}>
                                <Text style={styles.info_title}>Citas</Text>
                            </View>
                        </View>
                        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', width: '72%' }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text style={[styles.info_title, { marginRight: 10 }]}>{data.appointments.appointments_completed}</Text>
                                <Icon style={{ paddingVertical: 10, marginRight: 10 }} color={colors.primary} name="check" solid={true} size={20} />
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text style={[styles.info_title, { marginRight: 10 }]}>{data.appointments.appointments_appointments_goal}</Text>

                                <Icon style={{ paddingVertical: 10, marginRight: 10 }} color={colors.primary} name="flag" solid={true} size={20} />
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text style={[styles.info_title, { marginRight: 10 }]}>{Math.floor(data.appointments.appointments_percentage)}%</Text>
                ...
                customBody={() =>
                    ...
                    <View>
                        ...
                        {data.appointments.appointments.map((el: any, i: number) => {
                            ...
                            return (
                                <View key={el.id} style={{ paddingHorizontal: 10 }}>
                                    <View style={styles.tableItems}>
                                        ...
                                        <Text style={styles.text}>{el.is_done != 0 ? 'Asistió' : 'No Asistió'}</Text>
                                    </View>
                                </View>
                            )
                        })}
