...
export const CustomPolici = ({ data }: any) => {

    const colorsState = () => {

        let color
        switch (true) {
            case data.policies.policies_percentage <= 20:
                color = '#FF3729'
                break;
            case data.policies.policies_percentage <= 40:
                color = '#DAF7A6'
                break;
            case data.policies.policies_percentage <= 60:
                color = '#FF7F27'
                break;
            case data.policies.policies_percentage <= 80:
                color = '#F3DC06'
                break;
            case data.policies.policies_percentage == 100:
                color = 'green'
                break;
        }
        return color
            ...
            <AccordionItem
                    ...
                    <View style={{ flexDirection: 'row', height: '100%' }}>
                        <View style={{ alignItems: 'center', flexDirection: 'row', height: '100%' }}>
                            ...
                            <View style={{ paddingVertical: 10 }}>
                                <Text style={styles.info_title}>Pólizas</Text>
                            </View>
                        </View>
                        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', width: '68%' }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text style={[styles.info_title, { marginRight: 3 }]}>{data.policies.policies_completed}</Text>
                                <Icon style={{ paddingVertical: 10, marginRight: 10 }} color={colors.primary} name="check" solid={true} size={20} />
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text style={[styles.info_title, { marginRight: 3 }]}>{data.policies.policies_policies_goal}</Text>

                                <Icon style={{ paddingVertical: 10, marginRight: 10 }} color={colors.primary} name="flag" solid={true} size={20} />
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text style={[styles.info_title, { marginRight: 3 }]}>{Math.floor(data.policies.policies_percentage)}%</Text>
                ...
                customBody={() =>
                    ...
                    <View>
                        ...
                        {data.policies.policies.map((el: any, i: number) => {
                            ...
                            return (
                                <View key={el.id} style={{ paddingHorizontal: 10 }}>
                                    <View style={styles.tableItems}>
                                        ...
                                        <Text style={styles.text}>{el.status}</Text>
                                    </View>
                                </View>
                            )
                        })}
